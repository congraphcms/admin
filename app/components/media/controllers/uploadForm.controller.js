...
export default class FileFormController {
  ...
  init() {
    ...
    var FileCollection = fc.FileRepository.getCollection();
    fc.files = new FileCollection();
    fc.uploaderFiles = new FileCollection();

    fc.multiple = true;
    fc.multipleSelection = fc.multiple;

    // SELECTION
    // ------------------------
    fc.selectedItems = false;
  ...
  resolveForm() {
    this.$mdDialog.hide(this.uploaderFiles);
  }
